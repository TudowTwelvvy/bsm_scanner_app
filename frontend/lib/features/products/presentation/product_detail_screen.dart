import 'package:bsm_scanner_app/features/products/presentation/products_list_screen.dart';
...
//FutureProvider.autoDispose fetches the product when the screen opens
//and destroys it when the screen closes. This ensures fresh data every time.
final productDetailProvider = FutureProvider.autoDispose
    .family<ProductEntity?, int>((ref, productId) {
  // .family lets us pass parameters (productId) to the provider.
...
class _ProductDetailScreenState extends ConsumerState<ProductDetailScreen> {
  ...
  Future<void> _saveChanges(ProductEntity original) async {
    if (_nameController.text == original.productName &&
      ...
      // No changes made. Don't waste an API call.
    ...
    try {
      ...
      // Invalidate the list provider so it refreshes when we go back
        ...
        actions: [
          if (!_isEditing)
            IconButton(
              ...
              onPressed: () {
                // Pre-fill controllers when entering edit mode
